...
function Affiliateloginform() {
    ...
    const loginAffiliate = async (event) => {
        ...
        axios.post(`${process.env.REACT_APP_ROUTE}/api/brand/login`, {
            email: email,
            password: password
        }).then((res) => {
            if (res.data.error) {
    ...
    return (
        <div className='affiliate-landingpage-container'>
            <form onSubmit={loginAffiliate} className='affiliate-landingpage-form-container'>
                ...
                <p className='affiliate-landingpage-login-redirect'>Not registered yet? <Link to='/brand/register'>Create an Account</Link></p>
            </form>
